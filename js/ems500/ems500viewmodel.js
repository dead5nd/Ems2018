...
(function()
{
	...
	Ems500ViewModel.checkSrchResult = function(data)
	{
		...
		var i = 0;
		menjo[i] = {};
		menjo[i]['title'] = '地域枠フラグ';
		menjo[i++]['value'] = 'tiiki_ari';

		cmncode.template.bind("waku_Script", {"rows": menjo} , "waku_Tmpl");

		if (data.renkei['csv26'] == '1') { $("#tiiki_ari").prop('checked',true); }
		...
		//2次試験日
		$("#sikenbi2").val(data.renkei['csv19']);
		//大学入試センター試験成績請求番号
		$("#center_seikyu_no").val(data.renkei['csv28']);
    ...
    */
	Ems500ViewModel.submit = function(form)
	{
		...
		sendData['tiiki_ari'] = $("#tiiki_ari").prop('checked') ? '1': '';

		sendData['sikenti2_cd'] = $("#sikenti2_cd").val();
		sendData['sikenbi2'] = $("#sikenbi2").val();
		sendData['center_seikyu_no'] = $("#center_seikyu_no").val();

		//全データJSONの更新
		Ems500ViewModel.renkei['csv16'] = $("#sikenti_cd").val();
		Ems500ViewModel.renkei['csv19'] = $("#sikenbi2").val();
		...
		Ems500ViewModel.renkei['csv26'] = $("#tiiki_ari").prop('checked') ? '1': '';
